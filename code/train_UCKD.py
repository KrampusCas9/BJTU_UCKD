...
from UCKD import UCKD, SaveBestStudentModelCallback
...
def distilling_train(args):
    ...
    if args.train_mode == 'teacher-TA_1' or args.train_mode == 'TA_1-TA_2':
        teachers_paths.append(f"./ts_models/{Dataset_Name}/teacher/{args.data_type}/UCKD_best_model")
        teachers_paths.append(f'./ts_models/{Dataset_Name}/TA_1/{args.data_type}/UCKD_best_model')
        teachers_paths.append(f'./ts_models/{Dataset_Name}/TA_2/{args.data_type}/UCKD_best_model')
    else:
        teachers_paths.append(f"./ts_models/{Dataset_Name}/teacher/{args.data_type}/UCKD_best_model")
        teachers_paths.append(f'./ts_models/{Dataset_Name}/TA_1/{args.data_type}/UCKD_best_model')
        teachers_paths.append(f'./ts_models/{Dataset_Name}/TA_2/{args.data_type}/UCKD_best_model')
    ...
    if args.train_mode == 'teacher-TA_1':
        teacher_model_path = teachers_paths[0]
    ...
    elif args.train_mode == 'TA_1-TA_2':
        for i in range(2):
            teacher_model_path = teachers_paths[i]
            teacher_model = load_model(teacher_model_path)
            teacher_models.append(teacher_model)
    ...
    # 确定模型的保存路径
    model_path = f'./ts_models/{Dataset_Name}/{student_str}/{args.data_type}/UCKD_best_model'
    ...
    # 要保存的CSV文件路径
    csv_file_path = './result_csv/UCKD.csv'
    # 检查CSV文件是否存在
    ...
    dist = UCKD(student_model=student_model, teacher_models=teacher_models, train_mode=args.train_mode)
    dist.compile(
    ...
    record_experiment(args, fittedModel, test_loss, test_acc, csv_file='./result_csv/UCKD_result.csv')
